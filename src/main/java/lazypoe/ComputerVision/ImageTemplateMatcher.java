package lazypoe.ComputerVision;

import lazypoe.Utils;
...
public class ImageTemplateMatcher {
    ...
    private String stashPath = "assets/stash.jpg";

    private Template inventoryTemplate = new Template("Inventory", "green", inventoryPath, 0.9);
    private Template stashTemplate = new Template("Stash", "green", stashPath, 0.9);
    ...
    public boolean inventory(BufferedImage img) {
        return matchTemplateWithBI(inventoryTemplate, Utils.bufferedImageToMat(img), inventoryTemplate.getThreshold()).getCount() > 0;
    }

    public boolean stash(BufferedImage img) {
        return matchTemplateWithBI(stashTemplate, Utils.bufferedImageToMat(img), stashTemplate.getThreshold()).getCount() > 0;
    }
